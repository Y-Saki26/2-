...
実際の測定では電流-drivenの掃引により測定することが多く，この場合$J_n\rbra{{v_{\text{rf}}}K_{\text{J}}/{f}}$の整数倍でプラトーを持つ階段状のI-V特性が観測される（\reffig{shapiro-step}）．
理想的な接合を実際に作製することは難しいが，各ステップの幅はおよそBessel関数に従った形となる（\reffig{shapiro-bessel}）．

\addFigure[0.8\linewidth]{shapiro-step.png}{\ce{Nb}の点接触Josephson接合でのI-V特性\cite{josephson}．各曲線は入射するマイクロ波の強度（\si{dBm}）による特性の変化を表している．}{shapiro-step}
